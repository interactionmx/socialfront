    ...
    switch (action.type) {
      case 'usernameImmediately':
        ...
        if (draft.username.value.length > 30) {
          ...
          draft.username.message = 'User name should not exceed 30 characters'
        }
        if (draft.username.value && !/^([a-zA-Z0-9]+)$/.test(draft.username.value)) {
          draft.username.hasErrors = true
          draft.username.message = 'User name can only contain letters and numbers'
        }
        return
      case 'usernameAfterDelay':
        if (draft.username.value.length < 3) {
          draft.username.hasErrors = true
          draft.username.message = 'User name should be longer than 3 characters'
        }
      ...
      case 'passwordImmediately':
        ...
        if (draft.password.value.length > 50) {
          ...
          draft.password.message = 'password shoult not exceed 50 characters'
        }
        return
    ...
    <Page title="Welcome" wide={true}>
      <div className="row align-items-center">
        <div className="col-lg-7 py-3 py-md-5">
          <h1 className="display-3">Web Social App</h1>
